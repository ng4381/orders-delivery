...
public interface DeliveryOrderDetailRepository extends JpaRepository<DeliveryOrderDetail, Long> {
    ...
    /*
    @Query(nativeQuery = true, value = "select _dod.id as id, _dod.do_id as deliveryOrderId, _do.ext_order_id as deliveryOrderExtOrderId, p.id as productId, p.name as productName, \n" +
            "_dod.qty as total,\n" +
            "_dod.qty - (case when rd.qty is null then 0 else rd.qty end) as qty\n" +
    ...
    @Query(nativeQuery = true, value = "select _dod.id as id, _dod.do_id as deliveryOrderId, _do.ext_order_id as deliveryOrderExtOrderId, _dod.product_id as productId, p.name as productName, \n" +
            "_dod.qty as total,\n" +
            "_dod.qty - (case when rd.qty is null then 0 else rd.qty end) as qty\n" +
            "from deliveryorderdetail _dod\n" +
            "left join deliveryorder _do on _dod.do_id = _do.id\n" +
